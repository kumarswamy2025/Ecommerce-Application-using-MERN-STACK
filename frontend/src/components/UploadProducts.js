...
// update the UploadProducts state data
  ...
  const handleUploadProduct=async(e)=>{
    ...
    // here comment the  data.productName=file[0]?.name;  after custom upload
    // data.productName=file[0]?.name;
  ...
  const handleOnSubmit =async(e)=>{
    ...
    const API_data= await fetch(Apis.upload_product.API,{
      method:Apis.upload_product.method,
      credentials:"include",
      headers:{
  ...
  return (
    <div className='h-full w-full bg-slate-200 top-0 bottom-0 left-0 right-0 fixed bg-opacity-35 justify-center flex items-center'>
      <div className="bg-white p-4 w-full rounded max-w-2xl h-full max-h-[80%] overflow-y-scroll  custom-scrollbar">
        ...
        <h2 className='font-bold capitalize text-xl '> upload products  </h2>
        {/* close button */}
            ...
            <form  onSubmit={handleOnSubmit}>
             ...
             <div className='h-52 w-full bg-slate-200  cursor-pointer' >
              <div className='flex items-center flex-col justify-center pt-10'>
                 ...
                 <input required type="file" multiple id='upload_image_input'    className='hidden'  onChange={handleUploadProduct} />
              </div>
             ...
             <div className="pt-5">
              {
                ...
                data?.productImage && data?.productImage.length>0?(
                  ...
                  <p className='font-bold text-red-400 capitalize'>*please upload images </p>
                )
              }
             ...
             <div className='bg-sky-400 rounded-full p-2 mt-5 mb-5 justify-center flex hover:bg-sky-600'>
              <button className='w-full capitalize italic font-medium		'>submit</button>
             </div>
